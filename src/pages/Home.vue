...
<template>
    ...
    <Hero />
    <div class="lg:columns-4 md:columns-3 sm:columns-1 xl:columns-4 md:space-x-3 space-y-4 my-20  container">
            <ImageCard :id="image.id"  :profile="image?.profile_image?.small || ''" :name="image.user.name"
                ...
                :img="image?.urls?.regular" :handleModal="handleModal" v-for="image in images" :key="image.id" :likes="image.likes" />
    </div>
